...
public class BannerApplicationService {
    ...
    public Long createApplicationAndLock(CreateApplicationRequestDto req, Long userId) {
        ...
        // 2) LOCK 전환
        String in = slotIds.stream().map(id -> "?").collect(joining(","));
        jdbc.update("""
            UPDATE banner_slot
            SET status='LOCKED', locked_by=?, locked_until=NOW() + INTERVAL ? MINUTE
            WHERE slot_id IN ( %s )
        """.formatted(in), ps -> {
            ps.setLong(1, userId);
            ps.setInt(2, lockMinutes);
            for (int i=0;i<slotIds.size();i++) ps.setLong(3+i, slotIds.get(i));
        });
    ...
    public void cancelApplication(Long appId, Long userId) {
        ...
        """, Long.class, appId);
        if (!Objects.equals(owner, userId)) throw new SecurityException("권한 없음");
    ...
    public void markPaid(Long appId, Long adminId) {
        ...
        // SOLD 전환
        String in = slots.stream().map(m -> "?").collect(joining(","));
        jdbc.update("UPDATE banner_slot SET status='SOLD' WHERE slot_id IN ("+in+")",
                slots.stream().map(m -> m.get("slotId")).toArray());
        ...
        """, Statement.RETURN_GENERATED_KEYS);
            ...
            jdbc.update("UPDATE banner_slot SET sold_banner_id=? WHERE slot_id=?",
                    bannerId, (Long) m.get("slotId"));
        }
