...
public interface EventPopularityStatsCustomRepository {
        ...
        List<EventPopularityStatisticsListDto> aggregatedPopularity(LocalDate startDate, LocalDate endDate,
                        String mainCategory, String subCategory);

        TopEventsResponseDto topAggregatedPopularity(LocalDate startDate, LocalDate endDate);

        List<EventPopularityStatisticsListDto> searchEventWithRank(LocalDate startDate, LocalDate endDate,
                        String keyword,
                        String mainCategory, String subCategory);

        List<EventGenderPopularityDto> topMalesPick();

        List<EventGenderPopularityDto> topFemalesPick();

        List<EventAgePopularityDto> topAgePick(int age);
}
